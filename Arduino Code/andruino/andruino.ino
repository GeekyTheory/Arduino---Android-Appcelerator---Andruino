...
http://www.twitter.com/geekytheory
*/
#include <SPI.h>
#include <Ethernet.h>
...
void setup()
{
  ...
  state="OFF"; //Inicio con el LED apagado
}
void loop()
{
    ...
    while(cliente.connected())//Cliente conectado
    {
      if(cliente.available())
      {
        ...
        if(c=='\n' && lineaenblanco)//Si la petición HTTP ha finalizado
        {
          ...
          cliente.println("<input type=submit value=ON style=width:200px;height:75px onClick=location.href='./?LED=F\'>"); //Button
          cliente.println("<input type=submit value=OFF style=width:200px;height:75px onClick=location.href='./?LED=T\'>"); //Button
          cliente.println("</center>");
